 ...
 * --------------------------------------------------------
 * Run:
 *   node simulateCallbacks.js <PLAYER_ID> <GAME_UUID> <CURRENCY>
 * Example:
 *   node simulateCallbacks.js 68eb94c22a7983ea19b0bd6a 447eebaff41fdc9c36cf680720ea2ce321d33b67 USD
 */
...
const PLAYER_ID = process.argv[2] || "68eb94c22a7983ea19b0bd6a";
const GAME_UUID = process.argv[3] || "447eebaff41fdc9c36cf680720ea2ce321d33b67";
const CURRENCY = process.argv[4] || "USD";
// ===================

/**
 * Generate signed headers for Slotegrator-style callback
 */
...
/**
 * Send a single callback and log
 */
async function sendCallback(action, extra = {}) {
  ...
  try {
    ...
    const res = await axios.post(CALLBACK_URL, qs.stringify(body), config);
...
(async () => {
  ...
  await sendCallback("refund", { amount: 10, transaction_id: "TXN_REF_001", ref_transaction_id: "TXN_BET_001" });
  await new Promise((r) => setTimeout(r, 1000));

  await sendCallback("rollback", { amount: 15, transaction_id: "TXN_ROLL_001", ref_transaction_id: "TXN_BET_001" });
